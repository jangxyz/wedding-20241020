...
var default_scale = parseFloat(screen_scale)  // ÃÊ±â viewport °ª
...
$(function() {
    ...
    // È¥ÁÖµé ÀÌ¸§µéÁß ±æÀÌ°¡ ´Ù¸¥ ÀÌ¸§ÀÌ ÀÖ´ÂÁö Ã¼Å©
    ...
    if (Math.min(...parents_name_lengths) != Math.max(...parents_name_lengths)) {
        let parents_name_length_diff = 1  // ±æÀÌ°¡ ´Ù¸¥ ÀÌ¸§ÀÌ ÀÖÀ¸¸é align left Àû¿ë
    }


    // È¥ÁÖÀÌ¸§ÀÇ ±æÀÌ°¡ ¸ðµÎ °°Àº°æ¿ì
    if (parents_name_length_diff == 0) {

        // È¥ÁÖ ÀÌ¸§ ÀüÃ¼°¡ ÇÑ±ÛÀÎÁö Ã¼Å©
        let all_korean = false
        let parent_names = $('.insa_parent_block > .parent_name').text()
        if (/^[°¡-ÆR]*$/.test(parent_names)) {
            all_korean = true
        }
    ...
    $('.bank_family_group_block').each(function(){
        ...
        if (side_row_count == 0) {
            $('.bank_family_group_block').addClass('bank_family_group_block_'+side) // bank border ¼û±è
        } else {
    ...
    photo_view_swiper = new Swiper('#photo_view_swiper-container', {
        ...
        on: {
            slideChangeTransitionEnd: function() {
                ...
                photo_thmb_swiper.slideTo(page) // ÀÛÀº ÀÌ¹ÌÁö´Â ³ª´©±â 5
                if (photo_pop_swiper) {
    ...
    photo_thmb_swiper = new Swiper("#photo_thmb_swiper-container", {
        ...
        on: {
            slideChange: function() {
                // ½æ³×ÀÏ ±×·ì(5°³¹­À½) ÆäÀÌÂ¡ ¿À·ù È¸ÇÇ¿ë. ¿À·ù Ã£¾Æ³»¼­ ¼öÁ¤ ÇÊ¿ä
    ...
    function photo_pop_swiper_init() {
        photo_pop_swiper = new Swiper('#photo_pop_swiper-container', {
            ...
            on: {
                beforeInit: function() {},
                init: function() {
                    $('#photo_pic_pop_wrapper').hide() // ÃÊ±âÈ­ ÀÌÈÄ, »çÁø ÆË¾÷ ¿µ¿ª ¼û±è
                },
                slideChangeTransitionEnd: function() {
    ...
    // »çÁø »çÀÌÁî + À§Ä¡ Á¶Á¤
    var photo_pop_img_lr_margin = 30  // »çÁø ÁÂ¿ì ¸¶Áø
    var photo_pop_img_top_margin = 100  // »çÁø »ó´Ü ¿©¹é (»ç¿ë¾ÈÇÔ)
    ...
    family_group_name_prefix['a'] = '½Å¶û'
    family_group_name_prefix['b'] = '½ÅºÎ'
    ...
    $('.bank_txt_view_btn').on('click', function() {
        ...
        $('#bank_pop_title').text(family_group_name_prefix[$(this).data('family_group_type')] + 'Ãø °èÁÂ¹øÈ£')
    ...
    // °èÁÂ ¸ñ·ÏÀÌ Á¢ÇôÀÖ´Â »óÅÂÀÎÁö È®ÀÎ
    let fold_cnt = 0
    $('.bank_family_group_block > .family_bank_info_block').each(function(){
        if($(this).css('display') != 'none') {
            fold_cnt++  // ¸ñ·ÏÀÌ Á¢ÇôÀÖÁö ¾ÊÀº °æ¿ì Ä«¿îÆ®
        }
    })
    
    if (use_bank_fold == 1) { // °èÁÂ ¸ñ·Ï Á¢Èû»óÅÂ
        $('.family_members').children('.fold_arrow_btn_wrapper').children('.mcard_fold_arrow_btn').removeClass('on')  // Åä±Û ¹öÆ° v(Á¢Èû)
        $('.family_bank_info_block').hide()
    } else {
        $('.family_members').children('.fold_arrow_btn_wrapper').children('.mcard_fold_arrow_btn').addClass('on')   // Åä±Û ¹öÆ° ^ (ÆîÄ§)
    ...
    $('.family_members').on('click', function() {
        ...
        if ($(row_arrow).hasClass('on')) {
            $(row_arrow).removeClass('on')  // toggle È­»ìÇ¥ ¹æÇâ º¯°æ
            $(this).next('.family_bank_info_block').hide()  // °èÁÂ ³»¿ë ¼û±è 
        } else {
            $(row_arrow).addClass('on')
            $(this).next('.family_bank_info_block').show()  // °èÁÂ ³»¿ë º¸ÀÓ
        }
    })
    ...
    $('.mcard_fold_bar').on('click', function() {
        ...
        let bank_info_box = $(this).next('.bank_family_group_block')  // °èÁÂ¹øÈ£ Á¤º¸ ¿µ¿ª
    ...
    $('.msg_edit_button').live('click', function(e) {
        ...
        //¸Þ½ÃÁö ÆíÁýÆû
        $('#msg_form_title').text('´ñ±Û ¼öÁ¤')
        if (msg_area == 'guest' && msg_idx == msg_root) $('#msg_form_title').text('ÃàÇÏ ¸Þ½ÃÁö ¼öÁ¤')
        mask(0.5)
        $('#reply_form').css({
            'display': 'block',
    ...
    $('.msg_reply_button').live('click', function(e) {
        ...
        $('#msg_textarea').focus() //¶÷´ÙÇÔ¼ö »ç¿ë±ÝÁö

        if (!iOS) {
            //¿ø±ÛÀÌ Àß º¸ÀÌ°Ô ÀÚµ¿½ºÅ©·Ñ
            $('html, body').animate({
...
*/
    ...
    var notice_text_wrapper_hidden = false  // °øÁö³»¿ë ¹Ì³ëÃâ ¿©ºÎ °áÁ¤ ( false : ³ëÃâ, true: ¹Ì³ëÃâ)
    ...
    if($('#mcard_make_notice_block').length) {
        ...
        */
        ...
        $(window).on('scroll', function() {
            ...
            if(notice_text_wrapper_hidden == false) {  // °øÁö³»¿ë ¹Ì³ëÃâÀÌ ¾Æ´Ñ °æ¿ì
